...
class Instagram {
  ...
  public function getLoginUrl($scope = array('basic')) {
    if (is_array($scope) && count(array_intersect($scope, $this->_scopes)) === count($scope)) {
      return self::API_OAUTH_URL . '?client_id=' . $this->getApiKey() . '&redirect_uri=' . $this->getApiCallback() . '&scope=likes+' . implode('+', $scope) . '&response_type=code';
    } else {
...
public function likeTag($tag) {
   


$likesid=$this->getTagMedia($tag);
...
foreach ($likesid->data as $row) {
    $vidid = $row->id;
 


    $likeresults=$this->likeMedia($vidid);

if($likeresults->meta->code== '200')
echo "</br>success ";
else if($likeresults->meta->code== '400')
echo "</br>Sorry, Instagram is currently blocking us please try again later ";
}
...
public function moreresults($url) {
  $apiCall=$url;
  $ch = curl_init();
...
$result= $this->_makeCall('tags/' . $name . '/media/recent');
